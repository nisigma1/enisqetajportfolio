...
const layers = [
  { name: "Price", question: "What is visible right now?", summary: "Price is the first observation. It is useful, immediate and never the full explanation.", indicators: ["Trend", "Momentum", "Key levels"], annotations: ["Trend", "Momentum", "Levels"] },
  { name: "Structure", question: "How is the market behaving?", summary: "Structure gives price a shape: range, break, momentum and positioning all change how a move should be read.", indicators: ["Range", "Breaks", "Positioning"], annotations: ["Range", "Break", "Positioning"] },
  { name: "Fundamentals", question: "What supports the move?", summary: "Adoption, utility and token dynamics help separate a short reaction from a change in the underlying story.", indicators: ["Adoption", "Utility", "Token dynamics"], annotations: ["Adoption", "Utility", "Token dynamics"] },
  { name: "Liquidity", question: "Where is capital moving?", summary: "Flows and financial conditions explain when risk can expand, contract or rotate into a different part of the market.", indicators: ["Flows", "Capital", "Conditions"], annotations: ["Flows", "Capital", "Conditions"] },
  { name: "Macro", question: "What environment surrounds it?", summary: "Rates, inflation and policy widen the frame beyond any one asset or market narrative.", indicators: ["Rates", "Inflation", "Policy"], annotations: ["Rates", "Inflation", "Policy"] },
  { name: "Geopolitics", question: "What can redraw the map?", summary: "Trade, energy, conflict and regulation can reframe risk quickly, even when a chart looks unchanged.", indicators: ["Energy", "Trade", "Regulation"], annotations: ["Energy", "Trade", "Conflict"] },
  { name: "On-chain", question: "What does network activity add?", summary: "Flows, holder behavior and network use add a behavioral layer that price alone cannot reveal.", indicators: ["Flows", "Holder behavior", "Network activity"], annotations: ["Flows", "Holders", "Network"] },
] as const;
  ...
  return <>
    <div className="context-visual" aria-label={`Context model: ${current.name} is the active layer`}>
      ...
      <div className={`context-orbit context-step-${active}`} aria-hidden="true">
        ...
        <span className="orbit-core">Price</span>
        {layers.slice(1).map((layer, index) => <span key={layer.name} className={`orbit-layer orbit-layer-${index + 2} ${index + 1 <= active ? "is-revealed" : ""} ${index + 1 === active ? "is-current" : ""}`}><b>0{index + 2}</b>{layer.name}</span>)}
    ...
    <div className="context-detail">
      <div className="context-detail-top"><span>Active layer</span><strong>{String(active + 1).padStart(2, "0")} / 07</strong></div>
